...
/*   Updated: 2023/11/15 11:14:56 by incalero         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	**ft_create_array(char *av, int lines)
{
	...
	fd = open(av, O_RDONLY);
	if (fd == -1)
		return (f_ar(array), ft_printf ("Error, wrong file\n"), NULL);
	i = 0;
	while (i <= lines)
	{
		s = get_next_line(fd);
